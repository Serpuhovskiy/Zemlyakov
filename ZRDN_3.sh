...
cat "$maybeDestroyed" | while read line; do
    ...
    if [ $isMaybeDestroyedInCurrent -eq 0 ]; then
        ...
        source ./KP.sh "ZRDN_3" "$date" "$line" "Уничтожена" "$date ZRDN_3: Цель ID:$line УНИЧТОЖЕНА" 
    fi
done
...
if (( $(echo "$distance <= $R" | bc -l) )); then
    ...
    then
        ...
        if [ $x_1 != $x ] && [ $y_1 != $y ]; then
            ...
            if [ $speed -ge 250 ] && [ $speed -le 1000 ]; then
                ...
                # Отправляем инфу на КП
                source ./KP.sh "ZRDN_3" "$date" "$target_id" "Обнаружена" "$date ZRDN_3: Обнаружена цель (Крылатая ракета) ID:$target_id с координатами $x $y" 
                source ./KP.sh "ZRDN_3" "$date" "$target_id" "Выстрел" "$date ZRDN_3: Производится стрельба по цели ID:$target_id" 
            ...
            #  Если самолет
                ...
                # Отправляем инфу на КП
                source ./KP.sh "ZRDN_3" "$date" "$target_id" "Обнаружена" "$date ZRDN_3: Обнаружена цель (Самолет) ID:$target_id с координатами $x $y" 
                source ./KP.sh "ZRDN_3" "$date" "$target_id" "Выстрел" "$date ZRDN_3: Производится стрельба по цели ID:$target_id" 
    ...
    # Если количество > 1, значит две засечки уже было
        ...
        # Отправляем инфу на КП
        source ./KP.sh "ZRDN_3" "$date" "$target_id" "Промах" "$date ZRDN_3: Стрельба по цели ID:$target_id ПРОМАХ!" 
        source ./KP.sh "ZRDN_3" "$date" "$target_id" "Выстрел" "$date ZRDN_3: Повторная стрельба по цели ID:$target_id" 
